...
const ProjectCard: React.FC<Props> = ({ index, project }) => {
  ...
  useEffect(() => {
    ...
    tl.fromTo(
      ...
      .fromTo(q(".project-tags"), { y: -40 }, { y: 0, stagger: 0.1, ease: "Elastic.easeOut" }, "<25%");
  }, []);
      ...
      <div className={`project-card project-card-${index}`}>
        ...
        <div className="overflow-hidden">
          <div className="project-text flex items-center justify-between">
            ...
            <div className="flex items-center space-x-5 sm:space-x-3 my-2 sm:my-0 mr-[0.1rem]">
              ...
              >
                {/* GitHub icon */}
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" className="scale-150 sm:scale-125 opacity-75 hover:-rotate-12 fill-black dark:fill-bglight">
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
        ...
        >
          {project.tags.map((tag) => (
            <li key={tag} className="project-tags mr-2 my-1 bg-[#E2EFEF] dark:bg-carddark py-1 px-2 rounded">
              {tag}
            </li>
          ))}
        </ul>
